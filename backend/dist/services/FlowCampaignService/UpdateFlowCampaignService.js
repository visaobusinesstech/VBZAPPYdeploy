...
const UpdateFlowCampaignService = async ({ companyId, name, flowId, phrases, id, status, whatsappIds }) => {
    try {
        // Validações básicas
        if (!name?.trim()) {
        ...
        const duplicateCampaign = await FlowCampaign_1.FlowCampaignModel.findOne({
            where: {
                name: name.trim(),
                companyId,
        ...
        throw new AppError_1.default("Erro interno ao atualizar campanha", 500);
    }
};
