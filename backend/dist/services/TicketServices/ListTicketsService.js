    ...
    if (status === "closed") {
        ...
        if (!showTicketAllQueues && user.profile === "user") {
            latestTickets = await Ticket_1.default.findAll({
                attributes: ['companyId', 'contactId', 'whatsappId', [(0, sequelize_1.literal)('MAX("id")'), 'id']],
                where: {
                    [sequelize_1.Op.or]: [{ userId }, { status: ["pending", "closed", "group", "chatbot"] }],
            ...
            let whereCondition2 = {
                companyId,
                [sequelize_1.Op.or]: [{ userId }, { status: ["pending", "closed", "group", "chatbot"] }] // INCLUINDO CHATBOT NA BUSCA
            };
        ...
        whereCondition = {
            [sequelize_1.Op.or]: [
                ...
                {
                    status: showNotificationPendingValue ? { [sequelize_1.Op.in]: ["pending", "group", "chatbot"] } : { [sequelize_1.Op.in]: ["group", "chatbot"] },
