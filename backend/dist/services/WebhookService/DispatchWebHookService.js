...
const DispatchWebHookService = async ({ companyId, hashId, data, req }) => {
    try {
        ...
        if (webhook.config["details"]) {
            ...
            (0, ActionsWebhookService_1.ActionsWebhookService)(whatsappIds[0].id, //possivel bug aqui
            webhook.config["details"].idFlow, companyId, nodes, connections, nextStage, data, webhook.config["details"], hashId);
        }
        return webhook;
    }
    catch (error) {
