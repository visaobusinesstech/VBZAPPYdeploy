...
const ListService = async ({ companyId, searchParam = "", pageNumber = "1", kanban = 0, tagId = 0, limit: rawLimit }) => {
    let whereCondition = {};
    ...
    const offset = unlimited ? undefined : limit * (+pageNumber - 1);
    ...
    if (Number(kanban) === 0) {
        ...
        const hasMore = unlimited ? false : count > offset + tags.length;
        return {
            tags,
            count,
            hasMore
