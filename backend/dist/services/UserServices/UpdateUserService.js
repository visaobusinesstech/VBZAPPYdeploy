...
const UpdateUserService = async ({ userData, userId, companyId, requestUserId }) => {
    ...
    const { email, password, profile, name, queueIds = [], startWork, endWork, farewellMessage, whatsappId, allTicket, defaultTheme, defaultMenu, allowGroup, allHistoric, allUserChat, userClosePendingTicket, showDashboard, allowConnections, defaultTicketsManagerWidth = 550, allowRealTime, showContacts, showCampaign, showFlow, profileImage, finalizacaoComValorVendaAtiva, birthDate, allowSeeMessagesInPendingTickets } = userData;
    try {
    ...
    let processedBirthDate = user.birthDate;
