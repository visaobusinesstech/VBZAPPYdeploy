...
const GetGroupParticipantsService = async ({ contactId, companyId }) => {
    ...
    if (!contact.isGroup) {
        ...
        throw new AppError_1.default("Este contato não é um grupo", 400);
    }
    ...
    if (!remoteJid) {
        ...
        throw new AppError_1.default("Identificador do grupo não encontrado", 400);
    }
    ...
    try {
        if (contact.whatsappId) {
            ...
            wbot = await (0, wbot_1.getWbot)(contact.whatsappId);
        }
        else {
            ...
            wbot = await (0, wbot_1.getWbot)(defaultWhatsapp.id);
        }
        ...
        throw new AppError_1.default("WhatsApp não encontrado ou desconectado", 500);
    }
    try {
        ...
        const participantsPromises = groupMetadata.participants.map(async (participant) => {
            ...
            if (isLid) {
                ...
                try {
                    ...
                    if (lidMap && lidMap.contactId) {
                        ...
                        const contactByLid = await Contact_1.default.findOne({
                            where: {
                                companyId,
                                isGroup: false
                            }
                ...
                try {
                    ...
                    let existingContact = await Contact_1.default.findOne({
                        where: {
                            number,
                            companyId
                        }
        ...
        if (error.message?.includes("not_found") || error.message?.includes("item-not-found")) {
            ...
            throw new AppError_1.default("Grupo não encontrado no WhatsApp", 404);
        }
        ...
        if (error.message?.includes("Connection Closed") || error.message?.includes("not_connected")) {
            ...
            throw new AppError_1.default("WhatsApp desconectado", 503);
        }
        console.error(`[GetGroupParticipantsService] Unexpected error: ${error.message}`);
        throw new AppError_1.default(`Erro ao buscar participantes do grupo: ${error.message}`, 500);
    }
};
exports.default = GetGroupParticipantsService;
