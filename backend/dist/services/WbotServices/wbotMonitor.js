        ...
        wbot.ev.on("contacts.upsert", async (contacts) => {
            ...
            try {
                ...
                if (Array.isArray(filteredContacts)) {
                    ...
                    logger_1.default.info(`[RDS-BAILEYS] Pulando atualização de contatos - formato inválido para whatsapp ${whatsapp.id}`);
                }
            }
            catch (err) {
                logger_1.default.error(`[RDS-BAILEYS] Erro ao atualizar contatos para whatsapp ${whatsapp.id}:`, err);
                // Log apenas de um resumo, não dos contatos completos para evitar logs muito grandes
                logger_1.default.info(`[RDS-BAILEYS] Número de contatos tentados: ${filteredContacts?.length || 'undefined'}`);
            }
        });
    }
    catch (err) {
