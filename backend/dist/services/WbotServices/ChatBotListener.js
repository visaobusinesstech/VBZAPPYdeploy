...
const sendMessage = async (wbot, contact, ticket, body) => {
    ...
    if (debug_1.ENABLE_LID_DEBUG) {
        logger_1.default.info(`[RDS-LID] ChatBot - Enviando para JID tradicional: ${jid}`);
        logger_1.default.info(`[RDS-LID] ChatBot - Contact lid: ${contact.lid}`);
        logger_1.default.info(`[RDS-LID] ChatBot - Contact remoteJid: ${contact.remoteJid}`);
    }
    let sentMessage;
    if (ticket.isGroup) {
        if (debug_1.ENABLE_LID_DEBUG) {
            logger_1.default.info(`[RDS-LID] ChatBot - Enviando mensagem para grupo: ${jid}`);
        }
        try {
            ...
            if (debug_1.ENABLE_LID_DEBUG) {
                logger_1.default.info(`[RDS-LID] ChatBot - Sucesso na primeira tentativa para grupo ${jid}`);
            }
        }
        catch (err1) {
            if (debug_1.ENABLE_LID_DEBUG) {
                logger_1.default.warn(`[RDS-LID] ChatBot - Falha na primeira tentativa: ${err1.message}`);
            }
            // ✅ CORREÇÃO: Tratamento específico para erro de criptografia de grupo
            if (err1.message && err1.message.includes("senderMessageKeys")) {
                if (debug_1.ENABLE_LID_DEBUG) {
                    logger_1.default.error(`[RDS-LID] ChatBot - Erro de criptografia de grupo detectado: ${err1.message}`);
                }
                // Tentar enviar sem criptografia (último recurso)
                try {
                    if (debug_1.ENABLE_LID_DEBUG) {
                        logger_1.default.info(`[RDS-LID] ChatBot - Tentando envio sem criptografia para grupo ${jid}`);
                    }
                    sentMessage = await wbot.sendMessage(jid, {
                        text: (0, Mustache_1.default)(body, ticket)
                    });
                    if (debug_1.ENABLE_LID_DEBUG) {
                        logger_1.default.info(`[RDS-LID] ChatBot - Sucesso no envio sem criptografia para grupo ${jid}`);
                    }
                }
                catch (finalErr) {
                    if (debug_1.ENABLE_LID_DEBUG) {
                        logger_1.default.error(`[RDS-LID] ChatBot - Falha no envio sem criptografia: ${finalErr.message}`);
                    }
                    throw finalErr;
                }
            }
                ...
                if (debug_1.ENABLE_LID_DEBUG) {
                    logger_1.default.info(`[RDS-LID] ChatBot - Sucesso na segunda tentativa para grupo ${jid}`);
                }
            }
        }
    }
...
const sayChatbot = async (queueId, wbot, ticket, contact, msg, ticketTraking) => {
    ...
    if (String(selectedOption).toLocaleLowerCase() === "sair") {
        ...
        if (!(0, lodash_1.isNil)(complationMessage)) {
            ...
            await (0, wbotMessageListener_1.verifyMessage)(sendMsg, ticket, ticket.contact);
        }
        await (0, UpdateTicketService_1.default)({
            ticketData: {
                status: "closed",
