...
const SendWhatsAppMedia = async ({ media, ticket, body = "", isPrivate = false, isForwarded = false, quotedMsg = null }) => {
    try {
        const wbot = await (0, wbot_1.getWbot)(ticket.whatsappId);
        const companyId = ticket.companyId.toString();
        // ✅ CORREÇÃO: Verificar se body é válido antes de usar
        const safeBody = body || "";
        const formattedBody = ticket ? (0, Mustache_1.default)(safeBody, ticket) : safeBody;
        ...
        if (!media) {
            ...
            if (!formattedBody || formattedBody.trim() === "") {
                throw new AppError_1.default("ERR_EMPTY_MESSAGE");
            }
        ...
        if (media.mimetype === "application/octet-stream") {
            const detectedMime = mime_types_1.default.lookup(pathMedia);
            if (detectedMime) {
        ...
        if (ticket.isGroup) {
            ...
            try {
                sentMessage = await wbot.sendMessage((0, getJidOf_1.getJidOf)(ticket), options, quotedOptions);
            }
            catch (err1) {
        ...
        console.error(`❌ ERRO AO ENVIAR MÍDIA ${ticket.id} media ${media?.originalname || 'texto'}:`, err);
