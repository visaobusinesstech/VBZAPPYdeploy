...
const SendWhatsAppMediaFlow = async ({ media, ticket, whatsappId, body, isFlow = false, isRecord = false }) => {
    try {
        ...
        if (debug_1.ENABLE_LID_DEBUG) {
            logger_1.default.info(`[RDS-LID] SendWhatsAppMediaFlow - Enviando para JID tradicional: ${jid}`);
            logger_1.default.info(`[RDS-LID] SendWhatsAppMediaFlow - Contact lid: ${contact.lid}`);
            logger_1.default.info(`[RDS-LID] SendWhatsAppMediaFlow - Contact remoteJid: ${contact.remoteJid}`);
            logger_1.default.info(`[RDS-LID] SendWhatsAppMediaFlow - Media type: ${typeMessage}`);
        }
        const sentMessage = await wbot.sendMessage(jid, {
            ...options
        });
        if (debug_1.ENABLE_LID_DEBUG) {
            logger_1.default.info(`[RDS-LID] SendWhatsAppMediaFlow - Mídia enviada com sucesso para ${jid}`);
        }
