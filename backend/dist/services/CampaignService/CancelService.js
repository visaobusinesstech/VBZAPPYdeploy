...
async function CancelService(id) {
    ...
    if (campaign.tagListId && !campaign.contactListId) {
        ...
        try {
            ...
            const waitingJobs = await queues_1.campaignQueue.getWaiting();
            const delayedJobs = await queues_1.campaignQueue.getDelayed();
            const activeJobs = await queues_1.campaignQueue.getActive();
