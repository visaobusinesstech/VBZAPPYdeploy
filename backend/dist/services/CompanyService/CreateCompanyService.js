...
const validateCnpjWithReceita = async (cnpj) => {
    try {
        // Remover formatação do CNPJ
        const cleanCnpj = cnpj.replace(/\D/g, '');
        const response = await axios_1.default.get(`https://receitaws.com.br/v1/cnpj/${cleanCnpj}`);
...
const CreateCompanyService = async (companyData) => {
    const { name, phone, password, email, status, planId, dueDate, recurrence, document, paymentMethod, companyUserName, generateInvoice } = companyData;
