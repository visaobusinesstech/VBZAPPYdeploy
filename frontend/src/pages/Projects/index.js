            ...
            {data && data.length > 0 ? (
                data.filter(item => item.status === status).map(item => (
                    <Card key={item.id} style={{ marginBottom: 8 }}>
                        <CardContent>
                        <Typography variant="subtitle1">{item.title || "Sem título"}</Typography>
                        <Typography variant="body2" color="textSecondary">{item.description || "Sem descrição"}</Typography>
                        </CardContent>
                    </Card>
                ))
            ) : (
        ...
        <TableContainer component={Paper}>
            ...
            <TableBody>
                {data && data.length > 0 ? (
                    ...
                    <TableRow>
                        <TableCell colSpan={4} align="center">Nenhum projeto encontrado</TableCell>
                    </TableRow>
                )}
            </TableBody>
            </Table>
...
const ProjectsCalendar = ({ data }) => (
  <Paper style={{ padding: 16, height: '100%' }}>
    <Typography variant="h6">Calendário de Projetos</Typography>
    <div style={{ marginTop: 20, textAlign: 'center', color: '#666' }}>
      Componente de calendário será integrado aqui.
      {data && data.length > 0 && <div>{data.length} projetos carregados.</div>}
    </div>
  </Paper>
);

const ProjectsDashboard = ({ count }) => (
  <Grid container spacing={3}>
    ...
    <Grid item xs={12} sm={4}>
      <Paper style={{ padding: 16, textAlign: 'center' }}>
        ...
        <Typography variant="subtitle1">Atrasados</Typography>
      </Paper>
    </Grid>
    <Grid item xs={12} sm={4}>
      <Paper style={{ padding: 16, textAlign: 'center' }}>
        <Typography variant="h4" style={{ color: '#4caf50' }}>0</Typography>
        <Typography variant="subtitle1">Entregues</Typography>
      </Paper>
    </Grid>
  </Grid>
);
...
const Projects = () => {
  ...
  const { projects, loading, count } = useProjects({
      pageNumber: 1,
      searchParam: ""
  });
  ...
  const renderContent = () => {
    switch (activeTab) {
      case "dashboard":
        return <ProjectsDashboard count={count} />;
      case "board":
        return <ProjectsBoard data={projects} loading={loading} />;
      case "list":
        return <ProjectsList data={projects} loading={loading} />;
      case "calendar":
        return <ProjectsCalendar data={projects} />;
      default:
        return <ProjectsBoard data={projects} loading={loading} />;
    }
  };
    ...
    <MainContainer>
      <MainHeader>
        ...
        <MainHeaderButtonsWrapper>
          <Button variant="contained" color="primary">Novo Projeto</Button>
        </MainHeaderButtonsWrapper>
      </MainHeader>
