...
interface RegisterData {
  // Etapa 1 - Tipo de Pessoa e Identificação
  personType: 'fisica' | 'juridica'; // Pessoa Física ou Jurídica
  companyName: string;
  cnpj: string;
  ...
  targetAudience: string[]; // Múltipla escolha
  businessNiche: string;
  
  // Etapa 5 - Dados do Usuário
  name: string;
  position: string;
...
export default function Register() {
  ...
  const [plans, setPlans] = useState<Array<{key:string;name:string;url:string;limit:number|null;active?:boolean}>>([]);
  const [selectedPlan, setSelectedPlan] = useState<string | null>(null);
  const [planPaid, setPlanPaid] = useState<boolean>(false);
  ...
  useEffect(() => {
    const savedEmail = localStorage.getItem('vb_payment_email') || localStorage.getItem('vb_register_email');
    if (savedEmail) {
    ...
    const loadPlans = async () => {
      try {
        const { data } = await supabase
          ...
          .maybeSingle();
        const value = Array.isArray((data as any)?.value) ? (data as any)?.value : null;
        if (mounted) {
          setPlans((value || defaultPlans).filter((p:any)=>p.active!==false));
        }
      } catch {
  ...
  // Estado para controlar o carregamento da busca de CNPJ
  const [isFetchingCnpj, setIsFetchingCnpj] = useState(false);
  
  // Debounce para busca de CNPJ
  ...
  // Função para buscar dados do CNPJ pela API da Receita Federal
  const fetchCompanyByCNPJ = async (cnpj: string) => {
    ...
    try {
      ...
      // Usar a API do BrasilAPI que é mais confiável
      ...
      if (data.razao_social || data.nome_fantasia) {
        ...
        console.log('⚠️ CNPJ encontrado mas sem dados de nome');
      ...
      // Tentar API alternativa (ReceitaWS) em caso de erro
      try {
        console.log('🔄 Tentando API alternativa (ReceitaWS)...');
        const altResponse = await fetch(`https://www.receitaws.com.br/v1/cnpj/${cleanCNPJ}`);
        
        if (altResponse.ok) {
          const altData = await altResponse.json();
          console.log('📊 Dados da API alternativa:', altData);
          ...
          if (altData.nome || altData.fantasia) {
            ...
            console.log('✅ Dados encontrados na API alternativa:', {
              nome: altData.nome,
              fantasia: altData.fantasia
            });
          } else {
            setErrors(prev => ({ ...prev, cnpj: 'CNPJ válido mas sem dados disponíveis' }));
          }
  ...
  const handleInputChange = (field: keyof RegisterData, value: string | string[]) => {
    ...
    if (field === 'cnpj' && typeof value === 'string') {
      ...
      // Buscar dados da empresa quando CNPJ tiver 14 dígitos (com debounce)
  ...
  // Função para lidar com múltipla escolha de targetAudience
  ...
  const calculateCurrentStepErrors = () => {
    ...
    switch (currentStep) {
      case 0:
        if (!selectedPlan) newErrors.planKey = 'Selecione um plano';
        if (!planPaid) newErrors.planPaid = 'Conclua o pagamento para avançar';
        break;
      case 1: // Tipo de Pessoa e Identificação
      ...
      case 2: // CEP, Endereço e Telefone
      ...
      case 4: // Quantidade funcionários e Público-alvo
  ...
  const renderStep = () => {
    switch (currentStep) {
          ...
          <div className="space-y-3">
            <Card className="border-0 shadow-sm">
              <CardHeader className="pb-1 pt-3">
                <div>
                  <h3 className="text-sm font-semibold text-gray-900">Escolha seu plano</h3>
                  <Label className="text-xs text-gray-600">Selecione e realize o pagamento</Label>
                </div>
              </CardHeader>
              <CardContent className="pt-0 pb-3">
                <div className="grid grid-cols-2 gap-2">
                  {plans.map((p)=>(
                    <Button
                      key={p.key}
                      type="button"
                      variant={selectedPlan===p.key ? "default" : "outline"}
                      className={`justify-start ${
                        selectedPlan===p.key ? 'bg-blue-900 text-white border-blue-900' : 'border-gray-200 text-gray-700 hover:bg-blue-50'
                      }`}
                      onClick={()=>{
                        setSelectedPlan(p.key);
                        setFormData(prev=>({...prev, planKey: p.key}));
                      }}
                    >
                      {p.name}
                    </Button>
                ...
                {selectedPlan && (
                  <div className="mt-3 rounded-md border border-gray-200 p-3">
                    <p className="text-xs text-gray-600">
                      Limite de usuários: {plans.find(x=>x.key===selectedPlan)?.limit ?? '—'}
                    </p>
                  </div>
                )}
                <div className="mt-3 grid grid-cols-2 gap-2">
                  <Button
                    ...
                    onClick={()=>{
                      const plan = plans.find(x=>x.key===selectedPlan);
                      if (plan?.url) {
                  ...
                  <Button
                    type="button"
                    variant="outline"
                    disabled={!selectedPlan || !formData.email || isVerifyingPayment}
                    onClick={async ()=>{
                      setPaymentMessage('');
                      setIsVerifyingPayment(true);
                      try {
                        const email = String(formData.email || '').trim().toLowerCase();
                        const resp = await apiClient.getPaymentStatus({ email, planKey: selectedPlan || undefined });
                        const latest = resp?.data?.latest || null;
                        const confirmed = latest && String(latest.status || '').toLowerCase() === 'paid';
                        if (confirmed) {
                          setPlanPaid(true);
                          setFormData(prev=>({...prev, planPaid: true}));
                          try { localStorage.setItem('vb_payment_email', formData.email); } catch {}
                          setPaymentMessage('Pagamento confirmado com sucesso.');
                        } else {
                          setPaymentMessage('Pagamento não encontrado. Verifique o email usado no pagamento.');
                        }
                      } catch {
                ...
                {errors.planKey && <p className="text-red-500 text-xs mt-1">{errors.planKey}</p>}
                {errors.planPaid && <p className="text-red-500 text-xs mt-1">{errors.planPaid}</p>}
                {paymentMessage && (
                  <p
                    className={`text-xs mt-1 ${paymentMessage.includes('sucesso') ? 'text-green-600' : 'text-gray-600'}`}
                  >
                    {paymentMessage}
                  </p>
                )}
            ...
            <Card className="border-0 shadow-sm">
              <CardHeader className="pb-1 pt-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-sm font-semibold text-gray-900">Email usado no pagamento</h3>
                    <Label className="text-xs text-gray-600">Usaremos este email para confirmar o pagamento</Label>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="pt-0 pb-3">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      setFormData(prev => ({
                        ...prev,
                        personType: 'juridica',
                        cpf: '', // Limpar CPF ao trocar para Jurídica
                      }));
                  ...
                  <Button
                    ...
                    onClick={() => {
                      setFormData(prev => ({
                        ...prev,
                        personType: 'fisica',
                        cnpj: '', // Limpar CNPJ ao trocar para Física
                      }));
                      setErrors({}); // Limpar erros
                    }}
                  >
              ...
              <>
                ...
                <Card className="border-0 shadow-sm">
                  ...
                  <CardContent className="pt-0 pb-3">
                    ...
                    />
                    {isFetchingCnpj && (
                      <p className="text-blue-600 text-xs mt-1">Buscando dados da empresa...</p>
                    )}
                    {errors.cnpj && (
                ...
                <div className="flex flex-wrap gap-2">
                  {languages.map((lang) => (
                    <Button
                      ...
                      size="sm"
                      className={`text-xs ${
                        formData.defaultLanguage === lang.value 
                          ? 'bg-blue-900 text-white border-blue-900' 
                          : 'border-gray-200 text-gray-700 hover:bg-blue-50'
                      }`}
      ...
      case 4: // Quantidade funcionários, Target Audience e Nicho
        return (
          <div className="space-y-3">
            ...
            {/* Para quem vende (Múltipla escolha) */}
            <Card className="border-0 shadow-sm">
              <CardHeader className="pb-1 pt-3">
                <div className="flex items-center gap-3">
