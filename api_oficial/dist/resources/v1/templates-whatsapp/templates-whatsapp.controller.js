"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TemplatesWhatsappController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const templates_whatsapp_service_1 = require("./templates-whatsapp.service");
const auth_decorator_1 = require("../../../@core/guard/auth.decorator");
let TemplatesWhatsappController = class TemplatesWhatsappController {
    constructor(templatesService) {
        this.templatesService = templatesService;
    }
    async listTemplates(token) {
        return this.templatesService.listTemplates(token);
    }
    async getTemplate(token, templateName) {
        return this.templatesService.getTemplate(token, templateName);
    }
    async createTemplate(token, templateData) {
        return this.templatesService.createTemplate(token, templateData);
    }
    async deleteTemplate(token, templateName) {
        return this.templatesService.deleteTemplate(token, templateName);
    }
    async refreshTemplates(token) {
        return this.templatesService.refreshTemplates(token);
    }
};
exports.TemplatesWhatsappController = TemplatesWhatsappController;
__decorate([
    (0, auth_decorator_1.Public)(),
    (0, common_1.Get)(':token'),
    (0, swagger_1.ApiOperation)({ summary: 'Listar todos os templates da conta' }),
    (0, swagger_1.ApiResponse)({ status: 200, description: 'Lista de templates' }),
    (0, swagger_1.ApiResponse)({ status: 404, description: 'Conexão não encontrada' }),
    __param(0, (0, common_1.Param)('token')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], TemplatesWhatsappController.prototype, "listTemplates", null);
__decorate([
    (0, auth_decorator_1.Public)(),
    (0, common_1.Get)(':token/:templateName'),
    (0, swagger_1.ApiOperation)({ summary: 'Buscar template específico pelo nome' }),
    (0, swagger_1.ApiResponse)({ status: 200, description: 'Dados do template' }),
    (0, swagger_1.ApiResponse)({ status: 404, description: 'Template não encontrado' }),
    __param(0, (0, common_1.Param)('token')),
    __param(1, (0, common_1.Param)('templateName')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", Promise)
], TemplatesWhatsappController.prototype, "getTemplate", null);
__decorate([
    (0, auth_decorator_1.Public)(),
    (0, common_1.Post)(':token'),
    (0, swagger_1.ApiOperation)({ summary: 'Criar novo template' }),
    (0, swagger_1.ApiResponse)({ status: 201, description: 'Template criado' }),
    (0, swagger_1.ApiResponse)({ status: 400, description: 'Erro na criação' }),
    __param(0, (0, common_1.Param)('token')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, Object]),
    __metadata("design:returntype", Promise)
], TemplatesWhatsappController.prototype, "createTemplate", null);
__decorate([
    (0, auth_decorator_1.Public)(),
    (0, common_1.Delete)(':token/:templateName'),
    (0, swagger_1.ApiOperation)({ summary: 'Deletar template' }),
    (0, swagger_1.ApiResponse)({ status: 200, description: 'Template deletado' }),
    (0, swagger_1.ApiResponse)({ status: 404, description: 'Template não encontrado' }),
    __param(0, (0, common_1.Param)('token')),
    __param(1, (0, common_1.Param)('templateName')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", Promise)
], TemplatesWhatsappController.prototype, "deleteTemplate", null);
__decorate([
    (0, auth_decorator_1.Public)(),
    (0, common_1.Post)(':token/refresh'),
    (0, swagger_1.ApiOperation)({ summary: 'Atualizar cache de templates' }),
    (0, swagger_1.ApiResponse)({ status: 200, description: 'Cache atualizado' }),
    __param(0, (0, common_1.Param)('token')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], TemplatesWhatsappController.prototype, "refreshTemplates", null);
exports.TemplatesWhatsappController = TemplatesWhatsappController = __decorate([
    (0, common_1.Controller)('v1/templates-whatsapp'),
    (0, swagger_1.ApiTags)('Templates WhatsApp'),
    __metadata("design:paramtypes", [templates_whatsapp_service_1.TemplatesWhatsappService])
], TemplatesWhatsappController);
//# sourceMappingURL=templates-whatsapp.controller.js.map